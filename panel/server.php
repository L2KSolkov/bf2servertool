					...
					$.get('<?=HOME_URL?>panel/ajax/serverInfo.php', {players:true,chat:true}, function(data) {
						if(data.status.length > 0) {
							if(data.status == 'OK') {
								...
								'<p><b><?=$lang['word_players']?>:</b> ' + data.info.playersCurrent + ' / ' + data.info.playersMax + ' (' + data.info.playersJoining + ' <b class="red"><?=$lang['tool_server_joining']?></b>)</p>'+
								'<p><b><?=$lang['tool_server_curmap']?>:</b> ' + data.info.mapName + ' [' + data.info.gameModeName + ']</p>' +
